...
const sequelize = new Sequelize("pemesanan_hotel", "root", "", {
  host: "localhost",
  dialect: "mysql",
});
...
exports.addPemesanan = async (request, response) => {
  ...
  if (kamar === null) {
    ...
    let kamarCheck = await sequelize.query(
      `SELECT * FROM detail_pemesanans WHERE id_kamar = ${kamar.id} AND tgl_akses= "${request.body.tgl_check_in}" ;`
    );
